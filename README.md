...
- 🆕 = เทรนด์ที่เพิ่งขึ้นใหม่ · 🔺/🔻 = อันดับขึ้น/ลงจากรอบก่อน · ⏱️ = ติดเทรนด์มากี่ชั่วโมงในวันนี้
- 🚨 เตือนอัตโนมัติถ้าดึงเทรนด์ไม่ได้ (เว็บเปลี่ยนโครงสร้าง/เน็ตล่ม)
- 📊 สรุปเทรนด์ที่อยู่ยาวสุดประจำวัน (ส่ง 20:00 น. เวลาไทย)
...
## สั่งงานบอทจาก Telegram (พิมพ์คำสั่งเอง)

นอกจากส่งอัตโนมัติทุกชั่วโมง ยังพิมพ์คำสั่งขอเทรนด์เดี๋ยวนั้นได้ด้วย:

| คำสั่ง | ทำอะไร |
|--------|--------|
| `/now` | ดึงเทรนด์ทุกพื้นที่ที่ตั้งไว้เดี๋ยวนี้ |
| `/thailand` · `/worldwide` · `/japan` … | ดึงเฉพาะพื้นที่นั้น |
| `/help` | แสดงคำสั่งทั้งหมด |

**การทำงาน:** [.github/workflows/bot.yml](.github/workflows/bot.yml) เช็คข้อความใหม่ทุก ~5 นาที
แล้วตอบ (ไม่ใช่เรียลไทม์ — รอไม่กี่นาที) · บอทตอบเฉพาะ `CHAT_ID` ที่ตั้งไว้ ไม่ตอบคนอื่นที่มาทัก

> ⚠️ **ต้องเปลี่ยน repo เป็น Public** ก่อน ไม่งั้นการเช็คทุก 5 นาทีจะเกินโควต้านาทีฟรีของ GitHub
> (Private ได้ฟรีแค่ 2,000 นาที/เดือน) — วิธีเปลี่ยน: **Settings → General → เลื่อนล่างสุด Danger Zone →
> Change repository visibility → Public**. ปลอดภัย เพราะ `BOT_TOKEN`/`CHAT_ID` อยู่ใน Secrets
> (เข้ารหัส ไม่โผล่แม้ repo public) และ `config.ini` ไม่ถูก push อยู่แล้ว

**รันบนเครื่องตัวเองแทน (ตอบทันที แต่ต้องเปิดคอม):**

```powershell
.\.venv\Scripts\python.exe bot.py --serve
```

**ตั้งเมนูคำสั่งให้ขึ้นในแอป Telegram (ไม่บังคับ):** ทัก **@BotFather** → `/setcommands` → เลือกบอท →
วางข้อความนี้:

```
now - ดึงเทรนด์ทุกพื้นที่เดี๋ยวนี้
thailand - เทรนด์ไทย
worldwide - เทรนด์ทั่วโลก
help - คำสั่งทั้งหมด
```
...
| ส่งเข้า channel | `channel_id` | `CHANNEL_ID` | ส่งเข้า channel สาธารณะด้วย เช่น `@mytrends` (บอทต้องเป็น admin) |
